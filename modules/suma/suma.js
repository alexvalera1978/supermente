...
function mostrarExplicacion() {
    ...
    html += `Sumamos <strong>${num1}</strong> + <strong>${num2}</strong><br><br>`;
    html += `Empezamos por la derecha:<br>`;
    const s1 = num1.toString().split('').reverse();
    const s2sum = num2.toString().split('').reverse();
    let carry = 0;
    for (let i = 0; i < Math.max(s1.length, s2sum.length); i++) {
        const d1 = parseInt(s1[i] || 0);
        const d2 = parseInt(s2sum[i] || 0);
        const suma = d1 + d2 + carry;
        const digito = suma % 10;
        const nuevoCarry = Math.floor(suma / 10);
        html += `- ${d1} + ${d2}${carry > 0 ? ' + ' + carry + ' (llevamos)' : ''} = ${suma}`;
        if (suma >= 10) html += ` -> escribimos ${digito} y llevamos ${nuevoCarry}`;
        html += `<br>`;
        carry = nuevoCarry;
    }
    if (carry > 0) html += `- Escribimos el ${carry} que llevamos<br>`;
